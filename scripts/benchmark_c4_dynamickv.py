...
def _prefill_and_compress(
    ...
    model.set_attn_implementation("sdpa")
    with torch.no_grad():
        pref = model(prefix_ids, use_cache=True, logits_to_keep=1)
...
def generate_prepared_c4(
    ...
    *,
    ...
    """Generate with C4 from finalized IDs, preserving its cache behavior."""
    ...
    past, last_logits, stats = _prefill_and_compress(
        ...
        device=device,
        decode_headroom=max_new_tokens,
    )
    ...
    with torch.no_grad():
        ...
        for step_i in range(max_new_tokens):
            ...
            # Retained keys keep their original logical RoPE positions.
            pos = torch.tensor([[logical_len + step_i]], device=device)
            step = model(next_tok, past_key_values=past, use_cache=True,
                         position_ids=pos, logits_to_keep=1)
            n_decode_forwards += 1
    ...
    kv_bytes = stats["compressed_kv_bytes"] + n_decode_forwards * bpt
    kv_bytes_fp16 = stats["full_kv_bytes"] + fp16_decode_growth_bytes(
        stats["full_kv_bytes"], n_decode_forwards, bpt, model.config
    )
...
def _generate_dkv(
    ...
    prediction, _, stats = generate_prepared_c4(
        model,
        tokenizer,
        ids,
        max_new,
    ...
    legacy_stats.pop("n_decode_tokens")
